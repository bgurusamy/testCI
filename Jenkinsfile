 ...
 //tools { 
   //     maven 'Maven 3.3.9' 
    ...
    stages {
      stage('Setup') {
            steps {
                slackSend channel: '#visetest', color: 'good', message: "STARTED: ${JOB_NAME} ${BUILD_NUMBER} ${BUILD_URL}",baseUrl:'https://cim.slack.com/services/hooks/jenkins-ci/',teamDomain:'cim',token:'ldhDuSiSkvnLEeFLUPyWndJF'
            }
        }
        stage ('Initialize') {
            steps {
